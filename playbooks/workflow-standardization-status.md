...
|---|---|---|---|---|---|---|---|
| `AMTA-Management/spiderweb-services` | Done | Done | Pending | Done | Done | Pending | Branch protection is implemented via classic branch-protection API (equivalent outcome to rulesets, no ruleset object). |
| `AMTA-Management/spiderweb` | Done | Done | Pending | Done | Done | Pending | Branch protection is implemented via classic branch-protection API (equivalent outcome to rulesets, no ruleset object). |
...
2. Run the explicit Phase 1.4 smoke PR (`test/verify-workflow`) on each repo.
3. Decide whether to purchase GitHub Advanced Security for private personal repos, or accept those two security exceptions.
